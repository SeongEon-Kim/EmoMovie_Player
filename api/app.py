...
def create_app(test_config=None):
    ...
    def movieSelect():
        if "user" in session:
            objects = app.database.execute(
                "select title from movie_info"
            ).fetchall()  # db에서 영화 리스트 뽑아옴
            print(objects)
            return render_template(
                "select.html", mlist=objects, username=session["user"]["username"]
            )  # home.html에 반환
        else:
            return render_template("sign_up.html")
    ...
    def getMovie(title):
        ...
        objects = app.database.execute(
            f"select * from movie_info where title = '{title}'"
        ).fetchall()
        if len(objects) == 0:
            return "not found"
        objects = objects[0]
    ...
    def signup():
        if request.method == "POST":
            ...
            try:
                ...
                print(gender)
                obj = app.database.execute(
                    f"select * from subject_info where name='{username}' and age={age} and email='{email}';"
                ).fetchall()
                if len(obj) > 0:
    ...
    def timestamp():
        ...
        try:
            app.database.execute(
                f"INSERT INTO data(subjectID, movieTitle, param, white_time ,white_x, white_y, yellow_time,yellow_x, yellow_y, green_time, green_x, green_y) \
...
# export APP_CONFIG_FILE=/Users/gimseong-eon/Desktop/EMO_PLAYER_SE/api/config/product.py
